...
public class FragmenOffreetServiceValideretRefuserDemande extends Fragment {
    ...
    public void PopulateListeView(List<ServiceStatut> PostsFeedList){
        ...
        RecyclerView recyclerView = (RecyclerView) rootView.findViewById(R.id.offretServicesvaetrefDemande);

        recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));

        recyclerView.setAdapter(new OffreetServiceValideretRefuserAdapterActivityDemande(PostsFeedList,this.getContext()));
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.activity_fragmen_offreet_service_valideret_refuser_demande, container, false);

        final SharedPreferences sharedPref ;
        sharedPref = this.getContext().getSharedPreferences("shL",0);
        LogiSa = sharedPref.getString("log","a");
        myAPI =getAPI();
        Call<List<ServiceStatut>> call = myAPI.affServiceDemandeValideretrefuser(sharedPref.getInt("id",0));
